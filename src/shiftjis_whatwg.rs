...
include!(concat!(env!("OUT_DIR"), "/shiftjis_whatwg_tables.rs"));
...
pub fn encode_from_str<'a>(input: &str, output: &'a mut [u8]) -> EncodeResult<'a> {
    ...
    for (offset, c) in input.char_indices() {
        ...
        if output_i >= output.len() {
            ...
            // Special case 1
            output[output_i] = 0x5C;
            output_i += 1;
            input_i = offset + 1;
        } else if code == 0x203E {
            // Special case 2
            output[output_i] = 0x7E;
            output_i += 1;
            input_i = offset + 1;
        } else if code >= 0xFF61 && code <= 0xFF9F {
            // Special case 3
            output[output_i] = (code − 0xFF61 + 0xA1) as u8;
            output_i += 1;
            input_i = offset + 1;
        } else {
            if code == 0x2212 {
            ...
            if let Ok(ptr_i) = ENCODE_TABLE.binary_search_by_key(&code, |x| x.0) {
                if (output_i + 1) < output.len() {
                    ...
                    output[output_i] = lead;
                    output[output_i + 1] = trail;
                    output_i += 2;
                    input_i = offset + 1;
                } else {
                    break;
