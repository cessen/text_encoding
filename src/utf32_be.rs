...
pub fn encode_from_str<'a>(
    ...
    let _ = is_end; // Unused for encoding to utf32, so silence warning.
    ...
    for (offset, c) in input.char_indices() {
        if (output_i + 3) < out_buffer.len() {
            let mut code = (c as u32).to_be_bytes();
...
pub fn decode_to_str<'a>(input: &[u8], out_buffer: &'a mut [u8], is_end: bool) -> DecodeResult<'a> {
    ...
    while let Some(bytes) = itr.next() {
        ...
        // Do the decode.
        if let Some(code) =
            core::char::from_u32(u32::from_be_bytes([bytes[0], bytes[1], bytes[2], bytes[3]]))
        {
            // Encode to utf8.
            let mut buf = [0u8; 4];
